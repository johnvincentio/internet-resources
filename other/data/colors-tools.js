/* global APP */

(function() {
	'use strict';

	var data = APP.data.addData('ion-ios-color-filter-outline', 'Colors and Tools', 'colors');

	APP.data.addItem(
		data,
		'material-io.png',
		'https://material.io/guidelines/',
		'Material Design',
		'Material Design Guidelines'
	);
	APP.model.add(data);

	APP.data.addItem(
		data,
		'material-io.png',
		'https://material.io/color',
		'Material Design',
		'Material Design Color Tool'
	);
	APP.model.add(data);

	APP.data.addItem(
		data,
		'flatuicolors.png',
		'http://flatuicolors.com/',
		'Flat UI Colors 2',
		'A good starting point for choosing a flat design color for your next project'
	);
	APP.model.add(data);

	APP.data.addItem(
		data,
		'flatiucolors.ico',
		'https://www.flatuicolors.net/',
		'FlatUIColors.net',
		'FlatUIColors is a color palette that emerges from the current concept of flat design'
	);
	APP.model.add(data);

	APP.data.addItem(
		data,
		'materialpalette.jpg',
		'http://www.materialpalette.com/',
		'Material Palette',
		'Another great set of colors, inspired in material design. Generate and download your palette'
	);
	APP.model.add(data);

	APP.data.addItem(
		data,
		'color-hex.ico',
		'http://www.color-hex.com/',
		'Color Hex Color Codes',
		'Color-hex gives extensive information about colors'
	);
	APP.model.add(data);

	APP.data.addItem(
		data,
		'beautycolorcode.ico',
		'https://www.beautycolorcode.com/',
		'About Color Hex',
		'Color Hex is a free color tool providing information about any color'
	);
	APP.model.add(data);

	APP.data.addItem(
		data,
		'colorhunt.gif',
		'http://www.colorhunt.co/',
		'Colorhunt Palettes',
		"Don't know what colors to use for your website? Colorhunt helps you with beautiful color palettes"
	);
	APP.model.add(data);

	APP.data.addItem(
		data,
		'lolcolors.svg',
		'http://www.lolcolors.com/',
		'LOL Colors Palettes',
		'Curated color palettes inspiration. Another source for great color palettes'
	);
	APP.model.add(data);

	APP.data.addItem(
		data,
		'uigradients.png',
		'http://uigradients.com',
		'UI Gradients',
		'Collection of beautiful color gradients for you to choose from and export to your project'
	);
	APP.model.add(data);

	APP.data.addItem(
		data,
		'paletton.png',
		'http://paletton.com',
		'Paletton',
		'A very popular tool which allows you to create color combinations that play well together'
	);
	APP.model.add(data);

	APP.data.addItem(
		data,
		'0to255.png',
		'http://www.0to255.com/',
		'0to255',
		'A simple tool that helps you find variations of any color. Perfect for hovers, borders and gradients'
	);
	APP.model.add(data);

	APP.data.addItem(
		data,
		'css3factory.png',
		'http://www.css3factory.com/linear-gradients/',
		'CSS Gradient Generator',
		'CSS Gradient Generator'
	);
	APP.model.add(data);

	APP.data.addItem(
		data,
		'adobe.ico',
		'https://color.adobe.com/create/color-wheel',
		'Adobe Color Wheel',
		'Adobe Color Wheel'
	);
	APP.model.add(data);

	APP.data.addItem(
		data,
		'w3schools.ico',
		'https://www.w3schools.com/colors/colors_picker.asp',
		'w3schools Colors',
		'w3schools Colors'
	);
	APP.model.add(data);

	APP.data.addItem(data, 'coolers.png', 'https://coolors.co/', 'Coolers', 'Browse Color Schemes');
	APP.model.add(data);

	APP.data.addItem(data, 'colrd.ico', 'http://colrd.com/', 'Colrd', 'Create and share color inspiration');
	APP.model.add(data);

	APP.data.addItem(data, 'colourco.ico', 'http://www.colourco.de/', 'Colourcode', 'Create color combinations');
	APP.model.add(data);
})();
